...
import {
  ...
  getCategories,
  setCurrentAppAttr
} from "lib/actions/getAppAttributes.actions";
import {
  getCartId,
  getCartItems,
  getCartTotal,
  searchProductItem,
  getOrderId, makePayment
...
const LayoutHeader = React.memo((props: any) => {
  ...
  const { cart, orderId } = props.productsReducer;
  ...
  const renderMenuItems = (category: ICategoryValues) => (
    <Menu.Item
      key={category.name}
      onClick={() => props.setCurrentAppAttr(category)}
    >
    ...
    <Header className="main-header-container">
      ...
      <Header className="mini-header">
        ...
        >
          ...
          {isEmpty(categories.currentCategoryList) &&
          isEmpty(categories.data.rows)
            ? ""
            : !isEmpty(categories.currentCategoryList)
            ? categories.currentCategoryList.map(renderMenuItems)
            : !isEmpty(categories.data.rows) &&
              categories.data.rows.map(renderMenuItems)}
        </Menu>
        <Menu
          mode="horizontal"
          defaultSelectedKeys={["3"]}
        ...
        >
          {departments.data.map((department: IDepartmentValues) => (
            <Menu.Item
              key={department.name}
              onClick={() => props.setCurrentAppAttr(department, "department")}
            >
              <Link
                route="shop"
                params={{ department: slugify(department.name.toLowerCase()) }}
              >
                <a>{department.name}</a>
              </Link>
            </Menu.Item>
