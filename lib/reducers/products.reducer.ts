...
export default (state = initialState.products, action: IActions) => {
  switch (action.type) {
    ...
    case REMOVE_ITEM_SUCCESS:
        state.cart = {
          ...state.cart,
          items: state.cart.items.filter((item: any) => item.item_id !== action.data.item_id),
          loading: false
        };
        // console.log('reduce ===> ', state.cart.items.filter((item: any) => item.item_id !== action.data.item_id));
        return { ...state };
