...
import { openNotificationWithIcon, headers, getUserData } from "lib/utils";
import { isEmpty } from "lodash";

export const getAllProducts = (withFeatured = true, page?: number) => {
  const api = !page
    ? `${API_PREFIX}/products`
    : `${API_PREFIX}/products?page=${page}`;
  return (dispatch: any) => {
...
export const getProductWithAppAttr = (appAttr: any, page?: number) => {
  ...
  const attr =
    typeof appAttr.category_id !== "undefined" ? "inCategory" : "inDepartment";

  const api = !page
    ? `${API_PREFIX}/products/${attr || "inDepartment"}/${appAttr.category_id ||
        appAttr.department_id}`
    : `${API_PREFIX}/products/${attr || "inDepartment"}/${appAttr.category_id ||
        appAttr.department_id}?page=${page}`;
...
export const getCartId = () => {
  return (dispatch: Dispatch) => {
    axios
      .get(`${API_PREFIX}/shoppingcart/generateUniqueId`)
      .then(({ data }: any) => {
...
export const getCartItems = () => {
  return (dispatch: Dispatch) => {
    axios
      .get(`${API_PREFIX}/shoppingcart/${localStorage.getItem("cart_id")}`)
      .then(({ data }: any) => {
...
export const getCartTotal = () => {
  return (dispatch: Dispatch) => {
    axios
      .get(
        `${API_PREFIX}/shoppingcart/totalAmount/${localStorage.getItem(
          "cart_id"
        )}`
      )
      .then(({ data }: any) => {
...
export const saveForLater = (itemId: number) => {
  return (dispatch: Dispatch) => {
    axios
      .get(`${API_PREFIX}/shoppingcart/saveForLater/${itemId}`)
      .then(({ data }: any) => {
        if (isEmpty(data)) {
...
export const getSavedItems = () => {
  return (dispatch: Dispatch) => {
    axios
      .get(
        `${API_PREFIX}/shoppingcart/getSaved/${localStorage.getItem("cart_id")}`
      )
      .then(({ data }: any) => {
...
export const addToCart = (productData: IProduct, attributes: string) => {
  return (dispatch: Dispatch) => {
    ...
    if (localStorage.getItem("cart_id")) {
      return axios
        .post(`${API_PREFIX}/shoppingcart/add`, {
...
export const removeFromCart = (productData: IProduct) => {
  return (dispatch: Dispatch) => {
    axios
      .delete(`${API_PREFIX}/shoppingcart/removeProduct/${productData.item_id}`)
      .then(({ data }: any) => {
        if (isEmpty(data)) {
...
export const getCurrentProductItem = (id: number) => {
  return (dispatch: Dispatch) => {
    ...
    axios.get(`${API_PREFIX}/products/${id}`).then(({ data }: any) => {
...
export const getProductAttributes = (pid: number) => {
  return (dispatch: Dispatch) => {
    axios.get(`${API_PREFIX}/attributes/inProduct/${pid}`).then(({ data }) => {
...
export const getProductsReviews = (pid: number) => {
  return (dispatch: Dispatch) => {
    axios.get(`${API_PREFIX}/products/${pid}/reviews`).then(({ data }) => {
...
export const searchProductItem = (value: string) => {
  return (dispatch: Dispatch) => {
    axios
      .get(`${API_PREFIX}/products/search?query_string=${value}`)
      .then(({ data }) => {
...
export const getShippingRegions = () => {
  return (dispatch: any) => {
    axios.get(`${API_PREFIX}/shipping/regions`).then(({ data }: any) => {
...
export const getAllTaxes = () => {
  return (dispatch: Dispatch) => {
    axios.get(`${API_PREFIX}/tax`).then(({ data }: any) => {
      dispatch({ type: GET_ALL_TAXES, data });
    });
  };
};
...
export const getOrderId = () => {
  const params = {
    cart_id: localStorage.getItem("cart_id"),
    shipping_id: getUserData().accessToken
      ? getUserData().customer.shipping_region_id
      : 2,
    taxId: 2
  };
  return (dispatch: Dispatch) => {
    axios
      .post(`${API_PREFIX}/orders`, params, { headers })
      .then(({ data }: any) => {
        dispatch({ type: GET_ORDER_ID, data });
      })
